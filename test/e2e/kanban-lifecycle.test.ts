	...
	try {
		await client.command({
			...
			message: "Use the kanban tool: action=new, title='Write unit tests', priority='high'. " +
				"Then action=move, id=K1, status=doing. " +
				"Then action=done, id=K1. " +
				"Report the final card status.",
		}, 30_000);
		...
		const d = db_();
		const card = d.prepare("SELECT status FROM cards WHERE id = 'K1'").get() as any;
