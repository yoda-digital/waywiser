...
import { registerSignalProvider } from "../../../../extensions/proactive.js";
...
import { calendarSignalProvider } from "./signals.js";
...
export default async function calendarExtension(pi: ExtensionAPI): Promise<void> {
	...
	// Register proactive calendar signals (spec 08 §6): meeting-soon,
	// conflicts, overloaded-day. Runs SQL-only against the materialized
	// projection so SENSE stays zero-network.
	const unregisterSignals = registerSignalProvider(calendarSignalProvider);
